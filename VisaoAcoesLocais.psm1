...
# ============================================================
# EXECUTAVEIS EXTERNOS GRANDES (ContaSenhaLAPS, TransferidorInstseg, ...)
# ============================================================
# Executaveis de dezenas/centenas de MB ficam FORA do pacote do modulo
# Visao de proposito - se estivessem no FileList do manifesto, TODA
# atualizacao futura da ferramenta (mesmo sem nada a ver com eles)
# forcaria o Update-Module/auto-atualizacao a rebaixar tudo de novo pra
# cada tecnico, ja que Install-Module/Update-Module nao faz atualizacao
# incremental (decisao com o usuario, 2026-08-23, com ContaSenhaLAPS.exe -
# TransferidorInstseg.exe segue o mesmo padrao por ter tamanho parecido).
# Ficam soltos no compartilhamento, copiados pro cache local SO na
# primeira vez (ou quando o tamanho mudar - nova versao publicada la) -
# depois disso abrem a copia local sempre, sem tocar rede de novo.
$script:PastaOrigemFerramentasExternas = '\\POLICY-SERVER.tre-ma.gov.br\ScanZonas\Ferramentas'

function Invoke-AcaoAbrirExecutavelExternoComCache {
    <#
        Garante uma copia local de um executavel externo grande (copia/
        atualiza se o tamanho remoto for diferente do local, via
        Copy-ArquivoComRobocopy de VisaoPacotes.psm1) e abre. Devolve
    ...
    $caminhoOrigem = Join-Path $script:PastaOrigemFerramentasExternas $NomeArquivo
    ...
    $caminhoLocal = Join-Path $pastaCacheLocal $NomeArquivo

    $tamanhoRemoto = (Get-Item -LiteralPath $caminhoOrigem).Length
    $precisaCopiar = $true
    ...
    if ($precisaCopiar) {
        try {
            Copy-ArquivoComRobocopy -Origem $caminhoOrigem -Destino $caminhoLocal -NomePacote $NomeArquivo -AoAtualizarStatus $AoAtualizarStatus
            Unblock-File -LiteralPath $caminhoLocal -ErrorAction SilentlyContinue
        } catch {
            return [PSCustomObject]@{ Sucesso = $false; Mensagem = "Falha ao copiar $NomeArquivo para a maquina local: $($_.Exception.Message)" }
        }
    }
...
function Invoke-AcaoAbrirContaSenhaLaps {
    ...
    Invoke-AcaoAbrirExecutavelExternoComCache -NomeArquivo 'ContaSenhaLAPS.exe' -AoAtualizarStatus $AoAtualizarStatus
}
...
Export-ModuleMember -Function Get-CaminhoVncViewer, Get-CaminhoRcViewer, Get-CaminhoVncViewerAtual, Get-CaminhoRcViewerAtual, Set-CaminhoVncViewer, Set-CaminhoRcViewer, Open-VncViewer, Open-RcViewer, Start-PingContinuo, Invoke-AcaoAbrirContaSenhaLaps, Invoke-AcaoAbrirTransferidorInstseg, `
